"""RL fine-tune of PhiNet for the 4D dynamic-unicycle NMR-CBF.
...
Task-completion signal here is the **CBF deviation loss** `||u - u_nom||^2`
rather than the dbint progress-toward-goal reward. The two are roughly
equivalent for a goto-goal nominal: minimising deviation from `u_nom` is the
same as letting the nominal drive the agent to the goal whenever it is safe to
do so. The advantage is that this version is goal-agnostic — works for any
nominal — and avoids the need to specify a goal in the loss.

The CBF-QP constraint with φ is `Lf_B + LG_B·u + α·h_B + φ ≤ 0`. Since φ ≥ 0
already, the RL signal trades off:
  • hard safety penalty on h_raw violations
  • CBF-deviation cost on ||u - u_nom||^2  (replaces progress reward)
  • light L2 reg on φ to nudge slack down

Run:
    python scripts/dubins4d/finetune_phi_dubins4d.py rl_v1 --wandb
"""
...
# Initial-state sampler: uniform over (x, y, θ, v), rejecting states inside the
# obstacle. Matches the notebook's PhiNet training distribution.
_X_LO = jnp.array([-3.0, -3.0, -jnp.pi, 0.0])
_X_HI = jnp.array([3.0, 3.0, jnp.pi, 1.5])


def sample_x0(key: jax.Array, n: int) -> jax.Array:
    cands = _X_LO + jr.uniform(key, (n * 4, 4)) * (_X_HI - _X_LO)
    outside = jnp.sqrt(cands[:, 0] ** 2 + cands[:, 1] ** 2) > R_OBS + 0.1
...
def make_episode_loss(
    ...
    def episode_loss(phi_params, x0, eps, rng_key):
        ...
        def step(x, noise_key):
            ...
            # Task-completion proxy: deviation of the CBF-filtered control from
            # the nominal. For the goto-goal nominal this is equivalent to a
            # progress reward — whenever the nominal is admissible the QP
            # passes it through (||u - u_nom|| = 0), and φ inflations push us
            # away from the nominal (bad).
            cbf_loss = jnp.sum((u - u_nom) ** 2)

            reg = jnp.sum(phi ** 2)

            step_cost = w_safety * safety + w_cbf * cbf_loss + w_reg * reg
            return x_next, (step_cost, phi[0], safety, cbf_loss)

        _, (step_costs, phis, safeties, cbf_losses) = jax.lax.scan(step, x0, noise_keys)
        return step_costs.sum(), (phis.mean(), safeties.sum(), cbf_losses.mean())
...
def main(
    ...
    if not init_ckpt.exists():
        raise FileNotFoundError(
            ...
            "Run the supervised pretraining cell in dubins4d_cbf_compare.ipynb first."
        )
    ...
    episode_loss = make_episode_loss(episode_length, w_safety, w_cbf, w_reg)
    ...
    def train_step(phi_params, opt_state, x0s, eps, rng_keys):
        def batch_mean_loss(p):
            losses, (phis, safeties, cbf_losses) = batched_loss(p, x0s, eps, rng_keys)
            return jnp.mean(losses), (jnp.mean(phis), jnp.mean(safeties), jnp.mean(cbf_losses))

        (loss, (mean_phi, mean_safety, mean_cbf)), grads = jax.value_and_grad(
        ...
        return params_new, opt_state_new, loss, gnorm, mean_phi, mean_safety, mean_cbf
    ...
    for it in range(num_iterations):
        ...
        # Bias eps toward small values (curriculum-style)
        eps = jr.uniform(k_eps, (4,)) ** 5 * EPS_MAX
        ep_keys = jr.split(k_roll, episodes_per_iter)

        phi_params, opt_state, loss, gnorm, mean_phi, mean_safety, mean_cbf = train_step(
                ...
                _wandb.log(
                    {
                        ...
                        "finetune_phi/cbf_deviation": cbf_v,

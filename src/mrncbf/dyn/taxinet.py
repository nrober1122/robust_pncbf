...
from mrncbf.dyn.dyn_types import BState, Control, Disturb, HFloat, LFloat, PolObs, State, TState, VObs
from mrncbf.dyn.odeint import rk4, tsit5
from mrncbf.dyn.task import Task
from mrncbf.networks.fourier_emb import PosEmbed
from mrncbf.networks.mlp import mlp_partial
from mrncbf.networks.pol_det import PolDet
from mrncbf.networks.train_state import TrainState
from mrncbf.plotting.phase2d_utils import plot_x_bounds
from mrncbf.plotting.plotstyle import PlotStyle
from mrncbf.plotting.poly_to_patch import poly_to_patch
from mrncbf.qp.min_norm_cbf import min_norm_cbf
from mrncbf.utils.costconstr_utils import poly4_clip_max_flat
from mrncbf.utils.jax_types import Arr, BFloat, BoolScalar, FloatScalar, TFloat
from mrncbf.utils.jax_utils import jax_vmap, smoothmax
...
class Taxinet(Task):
    ...
    def __init__(self):
        self.umax = jnp.tan(jnp.deg2rad(12.0))
        self._dt = Taxinet.DT
        self.pos_wall = 1.0
    ...
    def nscbf_sample_bounds(self) -> Float[Arr, "2 nx"]:
        return np.array([(-2.0, 2.0), (-np.pi/3, np.pi/3)]).T
    
    def get_contour_x0_nscbf(self):
        n_pts = 80
        with jax.ensure_compile_time_eval():
            bounds = np.array([(-2.0, 2.0), (-np.pi/3, np.pi/3)]).T
            idxs = (0, 1)
    ...
    def f(self, state: State) -> State:
        self.chk_x(state)
        p, theta = state
        return jnp.array([0.1 * self.VEL * jnp.sin(theta), 0.0])

    def G(self, state: State):
        self.chk_x(state)
        GT = np.array([[0.0, self.VEL/self.L]])
        G = GT.T
        return G * self.umax
    ...
    def train_bounds(self) -> Float[Arr, "2 nx"]:
        return np.array([(-1.5, 1.5), (-np.pi/3, np.pi/3)]).T
        # return np.array([(0, 1.5), (0, np.pi/3)]).T
        # return np.array([(0.5, 0.8), (0.4, 0.6)]).T

    def contour_bounds(self) -> Float[Arr, "2 nx"]:
        return np.array([(-2.5, 2.5), (-np.pi/3, np.pi/3)]).T
    ...
    def in_ci_approx(self, state: State) -> BoolScalar:
        ...
        scale_factor = 1.2
        # scale_factor = 0.5
        in_right = x <= self.pos_wall - scale_factor * jnp.maximum(v, 0) ** 2
        in_left = x >= scale_factor * jnp.minimum(v, 0) ** 2 - self.pos_wall
    ...
    def _phase2d_setups(self) -> list[Task.Phase2DSetup]:
        return [Task.Phase2DSetup("phase", self.plot_phase, Task.mk_get2d([self.P, self.THETA]))]
    ...
    def get_ci_points(self):
        ...
        xs = self.pos_wall - scale_factor * np.maximum(vs, 0.0) ** 2
        all_xs += [xs]
        all_vs += [vs]

        vs = np.linspace(-2.0, 2.0)[::-1]
        xs = scale_factor * np.minimum(vs, 0.0) ** 2 - self.pos_wall
        all_xs += [xs]
        all_vs += [vs]
    ...
    def plot_phase(self, ax: plt.Axes):
        ...
        if self.nscbf:
            ...
            PLOT_YMIN, PLOT_YMAX = -np.pi/4, np.pi/4
        ...
        with open('src/pncbf/dyn/hj_gt_r2.pkl', 'rb') as f:
            data = pickle.load(f)
            target_values = data['target_values']
            coordinate_vectors = data['coordinate_vectors']
        ax.contour(coordinate_vectors[0], coordinate_vectors[1], target_values.T, levels=[0], **PlotStyle.ci_line)

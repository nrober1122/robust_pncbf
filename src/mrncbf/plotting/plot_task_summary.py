...
from mrncbf.plotting.plotter import Plotter
from mrncbf.utils.jax_utils import jax2np, jax_jit, jax_vmap, rep_vmap
...
def plot_task_summary(task: Task, plotter: Plotter, nom_pol=None):
    ...
    bbT_x, _, _, _, _ = jax2np(jax_jit(rep_vmap(sim.rollout_plot, rep=2))(bb_x))
    bbT_h = jax2np(rep_vmap(task.h, rep=3)(bbT_x))
    bb_V_nom = bbT_h.max(-1)

    b_x0_plot = task.get_plot_x0()
    bT_x_plot, _, _, _, _ = jax2np(jax_jit(jax_vmap(sim.rollout_plot))(b_x0_plot))
